...
const handleCategories = () => {
  const categories = categoriesData.map(category => {
    const products = productsData.filter(
      product => product.category === category.routeName
    );
    return {
      ...category,
      id: uuidv4(),
...
const handleProducts = categories => {
  const products = productsData.map(product => {
    const category = categories.find(
      category => category.routeName === product.category
    );
    return {
      ...product,
      id: uuidv4(),
      categoryId: category.id,
      newPrice: (product.originPrice * (100 - product.discount)).toFixed(2),
    };
  });
...
(() => {
  // fetch data
  const categoriesList = handleCategories();
  const productsList = handleProducts(categoriesList);
